...
const getStatName = (arr, minBaseStat) => {
  let objs = getBaseStatGreaterThan(arr, minBaseStat);
  let names = [];
  objs.forEach(obj => names.push(obj.stat.name));
  return names;
};
...
const evenOddNumericValues = (arr) => {
  return (arr.filter(val => typeof (val) === 'number')).map(num => num % 2 ? 'odd' : 'even');
};
