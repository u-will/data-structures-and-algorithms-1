...
const validatePhoneNumber = (phoneNumber) => {
  // This one took a sec. regex101.com was super useful to problem solve this one
...
const findTagNames = elements => {
  return elements.reduce((acc, str) => {
    ...
    closingTags = closingTags.map(closingTag => {
      let closingTagChars = closingTag.split('');
      closingTagChars = closingTagChars.slice(1, -1);
      return closingTagChars.join('');
    });
    return acc.concat(closingTags);
  }, []);
};
