...
const sortPeopleBetter = (arr) => {
  return arr.sort((a, b) => {
    if (a.lastName > b.lastName)
      ...
      return -1;
    // same last name, so tiebreaker is first name now
    else {
      if (a.firstName > b.firstName)
        return 1;
      else if (a.firstName < b.firstName)
        return -1;
      // same first name, so tiebreaker is age now
      else {
        return a.age - b.age;
      }
    }
...
const sortMeetingsByDay = (arr) => {
  const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']; // No meetings on weekends!
...
const sortSchedule = (arr) => {
  ...
  return arr.sort((a, b) => {
    if (days.indexOf(a.dayOfWeek) > days.indexOf(b.dayOfWeek))
      ...
      return -1;
    else // on the same day, list shorter meeting first
