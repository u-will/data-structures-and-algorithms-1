...
let biggerThanLuke = (arr) => {
  return arr.reduce( (people, person) => {
    if(person.mass > 77)
      return people ? `${people} - ${person.name}`: person.name;
    return people;
  }, '');
};
...
function checkWin(tic, tac, toe) {
  return tic === tac && tic === toe && tic !== '';
}
...
const detectTicTacToeWin = (board) => {
  ...
  // rows
  if(!win) win = checkWin(board[0][0], board[0][1], board[0][2]);
  if(!win) win = checkWin(board[1][0], board[1][1], board[1][2]);
  if(!win) win = checkWin(board[2][0], board[2][1], board[2][2]);

  // cols
  if(!win) win = checkWin(board[0][0], board[1][0], board[2][0]);
  if(!win) win = checkWin(board[0][1], board[1][1], board[2][1]);
  if(!win) win = checkWin(board[0][2], board[1][2], board[2][2]);

  // diag
  if(!win) win = checkWin(board[0][0], board[1][1], board[2][2]);
  if(!win) win = checkWin(board[2][2], board[1][1], board[0][0]);
