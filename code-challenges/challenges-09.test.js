...
const createServer = () => {
  ...
  app.get('/events', (req, res) => getCurrentEvents(req, res));
...
const mapCurrentEvents = () => {
  return currentEvents.news.map(event => new Event(event));
};
...
const returnNames = (arr) => {
  return arr.reduce((people, person) => {
    return people.concat(person.name);
  }, []);
};
...
const reversedString = (str) => {
  return str.split('').reduce((backwords, letter) => {
    return letter + backwords;
  }, '');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((totalChildren, character) => {
    ...
    return totalChildren; // we have to return the current count, else this data gets lost!
  }, 0);
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((primes, val) => {
    if (isPrime(val))
      return primes.concat(val);
    else
      return primes;
  }, []).length;
};
...
const extractStat = (statName, arr) => {
  let things = arr.reduce((acc, thing) => {
    // I sort of hacked my way to this solution, by reading the outputs
    // i am pretty confident this is not a great anwser.
    if (thing.stat.name === statName)
      return thing;
    else
      return acc;
  }, {});

  // I am not totally sure this is right. what happens if we have an empty object?
  return things ? things : null;
};
...
const extractChildren = (arr) => {
  // first get all the chars with an 'a' in their name
  let chars = arr.filter(character => {
    return /[a]/gi.test(character.name);
  });

  // now we can use that to compare as we reduce.
  return chars.reduce((children, person) => {
    if (person.children)
