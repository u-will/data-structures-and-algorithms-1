...
const firstLetters = (arr) => {
  return arr.reduce( (acc, val) => {
    return acc.concat( val.slice(0,1) );
  }, []);
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.reduce( (numbers, number) => {
    return numbers.concat(number.slice(1, 4) + number.slice(6, 9) + number.slice(10, 14));
  }, []);
};
...
const onlyOddChars = (str) => {
  return str.split('').map( (val, idx) => idx % 2 ? val : '').join('');
};
...
const allHappy = (arr) => {
  ...
  return arr.every( thing => thing.includes(':)') );
};
...
const findEvery = (arr, target) => {
  // return arr.reduce( (acc, val) => {
  //   return acc && val.includes(target);
  // }, true);

  return arr.every( thing => thing.includes(target) );
};
...
const unenrollBrook = (arr) => {
  return arr.reduce( (acc, inner) => {
    return acc.concat([inner.reduce( (acc, name) => {
      if(name.includes('Brook'))
        return acc;
...
const sortByDay = (arr) => {
  ...
  arr.forEach( dayEvent => {
    daysOfWeek.forEach( (day, idx) => {
      if( dayEvent.includes(day) )
        days[idx].push(dayEvent);
    });
  });
